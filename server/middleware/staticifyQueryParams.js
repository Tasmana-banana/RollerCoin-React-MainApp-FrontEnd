...
const staticify = (root, options) => {
	...
	const cachedMakeHash = memoizee((filePath) => {
		...
		let hash = crypto.MD5(fileStr).toString();
	...
	const getVersionedPath = (p) => {
		...
		fileNameParts.push(fileNameParts.pop());

		return `${path.posix.join(opts.pathPrefix, path.dirname(p), fileNameParts.join("."))}?v=${cachedMakeHash(absFilePath)}`;
	};
	...
	const stripVersion = (p) => {
		...
		if (fileNameParts.length >= 3 && fileNameHash.length === HASH_LEN && reResult && reResult[0] === fileNameHash) {
