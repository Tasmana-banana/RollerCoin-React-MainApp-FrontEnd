...
const OnlyNotAuthRoute = ({ children, isAuthorizedSocket, isAuthorizedNode, language, ...rest }) => {
	return (
		<Route
			{...rest}
			render={() =>
				!(isAuthorizedSocket && isAuthorizedNode) ? (
					...
					<Redirect
						to={{
							pathname: `${getLanguagePrefix(language)}/game`,
						}}
					/>
				)
			}
...
export default connect(mapStateToProps, null)(OnlyNotAuthRoute);
