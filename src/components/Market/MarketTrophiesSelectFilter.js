...
import "../../assets/scss/Market/MarketTrophiesSelectFilter.scss";
...
const MarketTrophiesSelectFilter = ({ options, paginationType, productsUpdate, t, isMobile, sortingOptions }) => {
	...
	return (
		<Row noGutters={isMobile} className="trophies select-filters-wrapper">
			<Col xs={12} lg={4}>
				<Row noGutters>
					<Select
						...
						options={sortByOptions.map((item) => ({ ...item, label: t(`market.TrophiesSort.${item.label}`) }))}
						defaultValue={{ ...sortByOptions[0], label: t(`market.TrophiesSort.${sortByOptions[0].label}`) }}
						isClearable={false}
						isSearchable={false}
					/>
				</Row>
				...
				<Col xs={{ size: 2, offset: 6 }}>
					<Row noGutters className="filter-short-container">
						<Select
							...
							defaultValue={QTY_ON_PAGE_OPTIONS[0]}
							isClearable={false}
							isSearchable={false}
						/>
					</Row>
...
MarketTrophiesSelectFilter.propTypes = {
...
export default withTranslation("Game")(connect(mapStateToProps, null)(MarketTrophiesSelectFilter));
