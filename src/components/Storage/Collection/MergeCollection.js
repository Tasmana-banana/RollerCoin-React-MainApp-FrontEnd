...
import "../../../assets/scss/Storage/Collection/MergeCollection.scss";

const mapStateToProps = (state) => ({
	language: state.game.language,
});

class MergeCollection extends Component {
	static propTypes = {
	...
	componentDidUpdate(_, prevState) {
		...
		if (prevState.collections.length !== collections.length && collections.length) {
			forceVisible();
		}
	}
	...
	getCollections = async (options, isLoadMore) => {
		...
		try {
			const json = await fetchWithToken(`/api/storage/collection/items?type=merge&sort=${sort}&sort_direction=${sortDirection}&skip=${skip}&limit=${limit}`, {
				method: "GET",
				...
				{!!collections && !!collections.length && (
					<Row noGutters={false} className="collection-container">
						{collections.map((item) => (
							<MergeItemCard key={item._id} item={item} activeCardID={activeCardID} toggleActiveCard={this.toggleActiveCard} />
						))}
					</Row>
				)}
				{isLoading && (
...
export default withTranslation("Storage")(connect(mapStateToProps, null)(MergeCollection));
