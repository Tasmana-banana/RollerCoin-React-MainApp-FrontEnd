...
import MergePartCard from "./MergePartCard";
...
const defaultOptions = {
	sort: PARTS_SORT_OPTIONS[0].value.sort,
	sort_direction: PARTS_SORT_OPTIONS[0].value.sort_direction,
	skip: 0,
	limit: 6,
};

const MergePartsPage = ({ wsReact, refreshAllData }) => {
	...
	const fetchData = async () => {
		setIsLoading(true);
		const apiUrl = `/api/storage/crafting-list?item_type=mutation_component&sort=${options.sort}&sort_direction=${options.sort_direction}&skip=${options.skip}&limit=${options.limit}${
	...
	return (
		<div className="merge-miners-page">
			<MergeSearchAndSortBar options={options} sortOptions={PARTS_SORT_OPTIONS} searchAndSortHandler={searchAndSortHandler} />
			{!!items && !!items.length && (
				<Row>
					{items.map((item) => (
						<MergePartCard item={item} key={item.id} refreshMainData={refreshMainData} />
					))}
				</Row>
			)}
...
MergePartsPage.propTypes = {
	wsReact: PropTypes.object.isRequired,
	refreshAllData: PropTypes.func.isRequired,
};

export default MergePartsPage;
